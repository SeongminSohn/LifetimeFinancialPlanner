...
function incomeEditPage() {
    ...
    useEffect(() => {
        if (!id) return
        axios.get(`http://localhost:10000/api/income-events/${id}`, { withCredentials: true })
            .then(res => {
                const data = res.data
                setFormData({
    ...
    const handleChange = e => {
        ...
        if (['name', 'initialAmount', 'inflationAdjustment', 'userPercentage', 'isSocialSecurity'].includes(name)) {
    ...
    const handleSubmit = async e => {
        ...
        await axios.put(
            `http://localhost:10000/api/income-events/${id}`,
            payload,
    ...
    return (
        <div className="total">
            ...
            <form onSubmit={handleSubmit} className="profileSetting">
                ...
                <div className="login">
                    <label htmlFor="isSocialSecurity">Social Security</label>
